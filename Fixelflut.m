...
of_stream_async_read_line_block_t  lineHandleBlock = ^bool(OFStream* stream, OFString* line, OFException* exception){
	if(line != nil){
...
of_stream_async_read_block_t fixelHandleBlock = ^bool(OFStream* stream, void *buffer, size_t length, OFException* exception){
	if (exception) {
...
of_tcp_socket_async_accept_block_t acceptPixelflutBlock = ^bool(OFTCPSocket* socket, OFTCPSocket* clientSocket, OFException* exception){
...
of_tcp_socket_async_accept_block_t acceptFixelflutBlock = ^bool(OFTCPSocket* socket, OFTCPSocket* clientSocket, OFException* exception){
	...
	[clientSocket asyncReadIntoBuffer: f exactLength: sizeof(fixelflutPackage) block: fixelHandleBlock];
